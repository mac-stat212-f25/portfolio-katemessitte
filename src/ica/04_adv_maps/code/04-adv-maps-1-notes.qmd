...
# If they don't match, transform to match mn_cities
mn_water   <- st_transform(mn_water, crs = st_crs(mn_cities))
...
ggplot() +
  # County boundaries (base layer)
  geom_sf(data = mn_counties, fill = NA, color = "black") +
  # City locations (points on top)
  geom_sf(data = mn_cities, color = "red", size = 2) +
  # White background + clean theme
...
ggplot() +
  ...
  scale_fill_gradient(
    name = "Elevation (m)",
    low = "darkgreen",   # lowest elevations
    high = "white"       # highest elevations
  ) +
  
  # County outlines (no fill so raster shows through)
  geom_sf(data = mn_counties, fill = NA, color = "black") +
  
  # Cities with population >= 10,000
  geom_sf(
    data = mn_cities |> dplyr::filter(Population >= 10000),
...
png(filename = "../figures/tc_map_zoom.png", width = 500, height = 500)
ggplot() +
    geom_raster(data = elev_df, aes(x = x, y = y, fill = elevation)) + 
    geom_sf(data = mn_counties, fill = NA, color = "black") + # county boundary layer
    geom_sf(data = mn_water, fill = "lightsteelblue1", color = "lightsteelblue1") + # NEW: river/lake layer
