...
# Top 10 genres by count (lump the rest into "Other")
top_genres <- df %>%
...
# Order genres by total count (for a tidy left→right read)
genre_order <- sum_df %>% group_by(genre_lumped) %>% summarise(total = sum(n), .groups = "drop") %>%
  arrange(desc(total)) %>% pull(genre_lumped)
...
p <- layout(
    ...
    buttons = list(
      list(method = "update",
           args = list(list(visible = count_vis),
                       list(yaxis = list(title = "Tracks (count)"))),
           label = "Counts"),
      list(method = "update",
  ...
  title = list(
    text = paste0(
      ...
      "<sup>Interactive stacked bars — switch between Counts and Percent within genre.</sup>"
    )
  )
) %>%
